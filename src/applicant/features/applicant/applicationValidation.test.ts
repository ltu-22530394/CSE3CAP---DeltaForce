...
describe('application validation', () => {
  ...
  it('accepts a complete application', () => {
    expect(hasApplicationErrors(validateCompleteApplication(validForm()))).toBe(
      false,
    )
  })
  ...
  it('accepts local and international phone numbers', () => {
    expect(validateApplicationStep(0, validForm()).phone).toBeUndefined()
    ...
    expect(
      validateApplicationStep(0, internationalPhoneForm).phone,
    ).toBeUndefined()

    const invalidPhoneForm = { ...validForm(), phone: '12345' }
    expect(validateApplicationStep(0, invalidPhoneForm).phone).toBe(
